...
create view mb_views.nft_listings as
  select
    l.nft_contract_id,
    l.token_id,
    l.market_id,
    ...
    l.receipt_id,
    l.kind,
    l.price / coalesce(c.denominator, 1) as price,
    l.currency,
    l.listed_by,
    l.unlisted_at,
    l.accepted_at,
    ...
    l.invalidated_at
  from nft_listings l
  left join currency_conversions c
  on l.currency = c.id;

create view mb_views.nft_offers as
  select
    o.nft_contract_id,
    o.token_id,
    o.market_id,
    o.approval_id,
    o.currency,
    o.offer_price / coalesce(c.denominator, 1) as offer_price,
    o.offered_by,
    o.offered_at,
    o.receipt_id,
    o.offer_id,
    o.referrer_id,
    o.referral_amount / coalesce(c.denominator, 1) as referral_amount,
    o.withdrawn_at,
    o.accepted_at,
    o.expires_at,
    o.invalidated_at,
...
create view mb_views.nft_earnings as
  select
    e.nft_contract_id,
    e.token_id,
    e.market_id,
    ...
    e.receiver_id,
    e.currency,
    e.amount / coalesce(c.denominator, 1) as amount,
    e.is_affiliate,
    e.is_mintbase_cut
  from nft_earnings e
  left join currency_conversions c
...
create view mb_views.auctions_with_offer as
  ...
  from mb_views.nft_listings l
    ...
    on l.metadata_id = m.id
    left join (
      select * from mb_views.nft_offers
...
create view mb_views.nft_activities as
  select
    ...
    a.action_sender,
    a.action_receiver,
    a.price / coalesce(c.denominator, 1) as price,
    a.currency,
    t.reference,
    t.reference_hash,
    t.copies,
...
create view mb_views.nft_notifications as
  select
    ...
    price as amount,
    currency
  from mb_views.nft_activities where kind = 'make_offer'
  union all select
    timestamp,
    receipt_id,
    action_receiver as account_id,
    ...
    price as amount,
    currency
  from mb_views.nft_activities where kind = 'sale'
  union all select
    timestamp,
    receipt_id,
  ...
  union all select
  	...
  	price as amount,
  	currency
  from mb_views.nft_activities where kind = 'transfer'
  union all select * from (
      ...
      e.amount,
      e.currency
    from mb_views.nft_earnings e
    left join mb_views.nft_offers o
  ...
  union all select
    ...
    price as amount,
    currency
  from mb_views.nft_activities where kind  = 'mint' and action_sender <> action_receiver
